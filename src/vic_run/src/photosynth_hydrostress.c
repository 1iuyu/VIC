...
{   
    ...
    double *ac_sha = veg_var->ac_sha;
    double *ag_sun = veg_var->ag_sun;
    double *ag_sha = veg_var->ag_sha;
    double *aj_sun = veg_var->aj_sun;
    double *aj_sha = veg_var->aj_sha;
    double *ap_sun = veg_var->ap_sun;
    double *ap_sha = veg_var->ap_sha;
    double *an_sun = veg_var->an_sun;
    double *an_sha = veg_var->an_sha;
    ...
    double lmr_sun[MAX_CANOPYS] = {0};
    double lmr_sha[MAX_CANOPYS] = {0};
    // 初始化光合作用和水分胁迫变量
    double LAIcanopy = 0.0;
    double nscaler_sun[MAX_CANOPYS] = {0};
    double nscaler_sha[MAX_CANOPYS] = {0};
    // 在循环之前检查是否是夜间
    bool is_night = true;
    for (i = 0; i < Ncanopy; i++) {
        if (aPAR_sun[i] > 0.0) {
    ...
    for (i = 0; i < Ncanopy; i++) {
        ...
        if (Ncanopy == 1) {
            nscaler_sun[i] = veg_var->ksun_vcmax;
            nscaler_sha[i] = veg_var->ksha_vcmax;
        }
        else {
            nscaler_sun[i] = exp(-f_N * LAIcanopy);
            nscaler_sha[i] = exp(-f_N * LAIcanopy);
        }
        // 维护呼吸
        double lmr25_sun = lmr25top * nscaler_sun[i];
        double lmr25_sha = lmr25top * nscaler_sha[i];

        if (veg_lib->Ctype == PHOTO_C3) {
            lmr_sun[i] = lmr25_sun * ft(Tfoliage, param.PHOTO_EL) * 
                        fth(Tfoliage, param.PHOTO_LMRHD, param.PHOTO_LMRSE, lmrc);
            lmr_sha[i] = lmr25_sha * ft(Tfoliage, param.PHOTO_EL) * 
                        fth(Tfoliage, param.PHOTO_LMRHD, param.PHOTO_LMRSE, lmrc);
        } 
        else {
            lmr_sun[i] = lmr25_sun * pow(2.0, (Tfoliage - 298.15) / 10.0); // Q10 = 2.0, 298.15K = 25C
            lmr_sun[i] /= (1.0 + exp(1.3 * (Tfoliage - 328.15))); // 高温抑制，328.15K = 55C
            lmr_sha[i] = lmr25_sha * pow(2.0, (Tfoliage - 298.15) / 10.0);
            lmr_sha[i] /= (1.0 + exp(1.3 * (Tfoliage - 328.15)));
        }
        // 低LAI时减少lmr
        lmr_sun[i] *= min((0.2 * exp(3.218 * LAI_z[i])), 1.0);
        lmr_sha[i] *= min((0.2 * exp(3.218 * LAI_z[i])), 1.0);
        // 光抑制
    ...
    if (is_night) {
        ...
        for (i = 0; i < Ncanopy; i++) {
            ...
            aj_sun[i] = 0.0;
            aj_sha[i] = 0.0;
            an_sun[i] = 0.0 - bsun * lmr_sun[i];
            an_sha[i] = 0.0 - bsha * lmr_sha[i];
            ap_sun[i] = 0.0;
            ap_sha[i] = 0.0;
        ...
        for (i = 0; i < Ncanopy; i++) {
            ...
            if (aPAR_sun[i] > 0.0) {
                ...
                vcmax_sun = vcmax25 * nscaler_sun[i] * ft(Tfoliage, param.PHOTO_EV) * 
                                        fth(Tfoliage, param.PHOTO_DV, vcmaxse, vcmaxc);
                jmax_sun = jmax25 * nscaler_sun[i] * ft(Tfoliage, param.PHOTO_EJ) * 
                                        fth(Tfoliage, param.PHOTO_DJ, jmaxse, jmaxc);
                tpu_sun = tpu25 * nscaler_sun[i] * ft(Tfoliage, param.PHOTO_ET) * 
                                        fth(Tfoliage, param.PHOTO_DT, tpuse, tpuc);
                
                vcmax_sha = vcmax25 * nscaler_sha[i] * ft(Tfoliage, param.PHOTO_EV) * 
                                        fth(Tfoliage, param.PHOTO_DV, vcmaxse, vcmaxc);
                jmax_sha = jmax25 * nscaler_sha[i] * ft(Tfoliage, param.PHOTO_EJ) * 
                                        fth(Tfoliage, param.PHOTO_DJ, jmaxse, jmaxc);
                tpu_sha = tpu25 * nscaler_sha[i] * ft(Tfoliage, param.PHOTO_ET) * 
                                        fth(Tfoliage, param.PHOTO_DT, tpuse, tpuc);
                ...
                if (veg_lib->Ctype == PHOTO_C4) {
                    ...
                    vcmax_sun = vcmax25 * nscaler_sun[i] * temp_factor_sun;
                    ...
                    vcmax_sha = vcmax25 * nscaler_sha[i] * temp_factor_sha;
                }
                
                kp_sun = kp25 * nscaler_sun[i] * pow(2.0, (Tfoliage - (CONST_TKFRZ + 25.0)) / 10.0);
                kp_sha = kp25 * nscaler_sha[i] * pow(2.0, (Tfoliage - (CONST_TKFRZ + 25.0)) / 10.0);
                ...
                // 计算叶面CO₂分压cs和最终ci
                // 阳叶
                cs_sun = atmosCO2 - 1.4 / gb_mol * an_sun[i] * pressure;
                cs_sun = max(cs_sun, param.PHOTO_MAXCS);
                
                ci_sun = atmosCO2 - veg_var->an_sun[i] * pressure *
                    (1.4 * gs_mol_sun + 1.6 * gb_mol) /
                    (gb_mol * gs_mol_sun);
                ci_sun = max(ci_sun, 1.0e-6);
                
                // 阴叶
                cs_sha = atmosCO2 - 1.4 / gb_mol * an_sha[i] * pressure;
                cs_sha = max(cs_sha, param.PHOTO_MAXCS);
                
                ci_sha = atmosCO2 - an_sha[i] * pressure *
                ...
                veg_var->PhotoErr_sun = veg_lib->medlynslope * max(an_sun[i], 0.0) * hs / cs_sun *
                                pressure + max(bsun * veg_lib->medlynint, 1.0);
                
                // 阴叶
                hs = (gb_mol * ceair + gs_mol_sha * Qair_over) / ((gb_mol + gs_mol_sha) * Qair_over);
                
                veg_var->PhotoErr_sha = veg_lib->medlynslope * max(an_sha[i], 0.0) * hs / cs_sha *
    ...
    double RS_sha = 0.0;
    for (i = 0; i < Ncanopy; i++) {
        gscan_sun += LAIsun_z[i] / (Ra_sun[i] + Ra_leaf);
        laican_sun += LAIsun_z[i];
    }
    if (laican_sun > 0.0) {
        RS_sun = laican_sun / gscan_sun - Ra_leaf;
    }
    else {
        RS_sun = 0.0;
    }
    ...
    for (i = 0; i < Ncanopy; i++) {
        gscan_sha += LAIsha_z[i] / (Ra_sha[i] + Ra_leaf);
        laican_sha += LAIsha_z[i];
    }
    if (laican_sha > 0.0) {
        RS_sha = laican_sha / gscan_sha - Ra_leaf;
    }
    else {
        RS_sha = 0.0;
    }
    ...
    veg_var->RS_sun = RS_sun;
    veg_var->RS_sha = RS_sha;
