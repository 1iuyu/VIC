                  ...
                  soil_con_struct *soil_con)
{
    ...
    if (options.STATE_FORMAT == BINARY) {
        Nbytes = cell[0].Nnode * sizeof(double) + // dz_node
                 cell[0].Nnode * sizeof(double) + // Zsum_node
                 (Nveg + 1) * 2 * sizeof(int) +   // veg & band
                 (Nveg + 1) * options.Nsoil * sizeof(double) + // soil moisture
                 (Nveg + 1) * options.Nsoil * sizeof(double) + // soil ice
        ...
        Nbytes += (Nveg + 1) * options.Nsoil *sizeof(double) + // SnowFrac
                  ...
                  (Nveg + 1) * (cell[0].Nnode) * sizeof(double) + // soil temperatures
    ...
    for (nidx = 0; nidx < options.Nsoil; nidx++) {
        if (options.STATE_FORMAT == BINARY) {
    ...
    for (veg = 0; veg <= Nveg; veg++) {
        ...
        for (lidx = 0; lidx < options.Nsoil; lidx++) {
        ...
        for (nidx = 0; nidx < cell[veg].Nnode; nidx++) {
            if (options.STATE_FORMAT == BINARY) {
