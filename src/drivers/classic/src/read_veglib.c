            ...
            size_t *Ntype)
{
    ...
    while (!feof(veglib)) {
        if (str[0] <= 57 && str[0] >= 48) {
            ...
            fscanf(veglib, "%lf", &temp[i].a);      /**< Empirical parameter a in eqa(2) */
            fscanf(veglib, "%lf", &temp[i].b);      /**< Empirical parameter b in eqa(2) */
            fscanf(veglib, "%lf", &temp[i].d);      /**< Maximum root depth (m) */
