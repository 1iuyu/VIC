                         ...
                         soil_con_struct *soil_con)
{
    ...
    if (soil_con->Zsum_soil[options.Nsoil - 1] - soil_con->dp > DBL_EPSILON) {
        log_warn("Sum of soil nodes (%f) exceeds defined damping depth (%f)."
                 "Resetting damping depth.",
                 soil_con->Zsum_soil[options.Nsoil - 1], soil_con->dp);
        soil_con->dp = soil_con->Zsum_soil[options.Nsoil - 1];
    }
