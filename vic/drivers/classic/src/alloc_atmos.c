            ...
            force_data_struct **force)
{
    ...
    *force = calloc(nrecs, sizeof(force_data_struct));
    ...
    for (i = 0; i < nrecs; i++) {
        ...
        (*force)[i].Qair = calloc(NR + 1, sizeof(*(*force)[i].Qair));
        check_alloc_status((*force)[i].Qair, "Memory allocation error.");
        (*force)[i].rel_humid = calloc(NR + 1, sizeof(*(*force)[i].rel_humid));
        check_alloc_status((*force)[i].rel_humid, "Memory allocation error.");
