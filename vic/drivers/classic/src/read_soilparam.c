    ...
    if (!(*MODEL_DONE) && (*RUN_MODEL)) {
        ...
        *************************************************/
        ...
        if (!options.EXP_TRANS) {
            ...
            Bexp = logf(dp + 1.) / (double) (Nnode - 1);
            // validate Nnode by requiring that there be at
            // least 3 nodes in the top 50cm
            if (Nnode < 5 * logf(dp + 1.) + 1) {
                log_err("The number of soil thermal nodes (%zu) "
                        ...
                        Nnode, dp, (int) (5 * logf(dp + 1.)) + 2,
                        exp(0.2 * (Nnode - 1)) + 1);
            }
            for (k = 0; k <= Nnode - 1; k++) {
                temp->Zsum_soil[k] = expf(Bexp * k) - 1.;
            }
            if (temp->Zsum_soil[0] > temp->depth[0]) {
                log_err("Depth of first thermal node (%f) in "
                        ...
                        "dp (%f)", temp->Zsum_soil[0], temp->depth[0], dp);
            }

            // top node
            k = 0;
            temp->dz_soil[k] = temp->Zsum_soil[k + 1] - temp->Zsum_soil[k];
            // middle nodes
            for (k = 1; k < Nnode - 1; k++) {
                temp->dz_soil[k] =
                    (temp->Zsum_soil[k + 1] - temp->Zsum_soil[k]) / 2. +
                    (temp->Zsum_soil[k] - temp->Zsum_soil[k - 1]) / 2.;
            }
            // bottom node
            k = Nnode - 1;
            temp->dz_soil[k] = temp->Zsum_soil[k] - temp->Zsum_soil[k - 1];
        } // end if !EXP_TRANS
        /******************************************
