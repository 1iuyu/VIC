    ...
    if (Nbands > 1) {
        ...
        for (band = 0; band < Nbands; band++) {
            if (soil_con->AreaFract[band] > 0) {
                soil_con->Tfactor[band] = (soil_con->BandElev[band] - soil_con->elevation) * param.LAPSE_RATE;
            }
            else {
        ...
        for (band = 0; band < Nbands; band++) {
            if (soil_con->AreaFract[band] > 0) {
                soil_con->Pfactor[band] = (1.0 + param.SNOW_PGRAD * (soil_con->BandElev[band] - soil_con->elevation)) * 
