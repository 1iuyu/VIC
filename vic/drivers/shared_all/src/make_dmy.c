...
make_dmy(global_param_struct *global)
{
    ...
    if (global->nrecs == 0) {
        ...
        end_dmy.dayseconds = SEC_PER_DAY - global->step_dt;
        ...
        offset =
            (unsigned int) ((double) (SEC_PER_DAY -
                                      start_dmy.dayseconds) / global->step_dt);
        if ((((unsigned int) global->step_dt *
    ...
    for (i = 0; i < global->nrecs; i++) {
        dt_seconds_to_time_units(global->time_units, i * global->step_dt,
                                 &dt_time_units);
