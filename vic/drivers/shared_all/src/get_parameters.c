    ...
    while (!feof(paramfile)) {
        if (cmdstr[0] != '#' && cmdstr[0] != '\n' && cmdstr[0] != '\0') {
            ...
            *************************************/
            ...
            else if (strcasecmp("SVP1", optstr) == 0) {
                sscanf(cmdstr, "%*s %lf", &param.SVP1);
            }
            else if (strcasecmp("SVP2", optstr) == 0) {
                sscanf(cmdstr, "%*s %lf", &param.SVP2);
            }
            else if (strcasecmp("SVP3", optstr) == 0) {
                sscanf(cmdstr, "%*s %lf", &param.SVP3);
            }
...
validate_parameters()
{
    ...
    // Saturation Vapor Pressure Parameters
    if (!(param.SVP1 >= 0.)) {
        log_err("SVP1 must be defined on the interval [0, inf) (kPa)");
    }
    if (!(param.SVP2 >= 0.)) {
        log_err("SVP2 must be defined on the interval [0, inf) (kPa)");
    }
    if (!(param.SVP3 >= 0.)) {
        log_err("SVP3 must be defined on the interval [0, inf) (kPa)");
    }
    // Photosynthesis Parameters
