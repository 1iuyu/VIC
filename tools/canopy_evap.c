            ...
            veg_var_struct    *veg_var)
{
    ...
    if (energy->FrozenOver == true) {
        ...
        canopy_vapor = 0.0;
        canopydew = 0.0;
        canopy_sublim = max(LatentLeaf / CONST_LATSUB, 0.0);
        canopyfrost = fabs(min(LatentLeaf / CONST_LATSUB, 0.0));
    }
    else {
        transp = max(LatentTransp / CONST_LATVAP, 0.0);
        canopy_vapor = max(LatentLeaf / CONST_LATVAP, 0.0);
        canopydew = fabs(min(LatentLeaf / CONST_LATVAP, 0.0));
        canopy_sublim = 0.0;
        canopyfrost = 0.0;        
    }
    ...
    /* phase change */
    if (int_snow > param.TOL_A && Tfoliage > CONST_TKFRZ) {
        canopy_melt = min(int_snow / step_dt, (Tfoliage - CONST_TKFRZ) * CONST_CPICE *
        ...
        Tfoliage = wetFrac * CONST_TKFRZ + (1.0 - wetFrac) * Tfoliage;
    }

    /* canopy water refreeezing */
    if (int_rain > param.TOL_A && Tfoliage < CONST_TKFRZ) {
        canopy_freez = min(int_rain / step_dt, (CONST_TKFRZ - Tfoliage) * CONST_CPFW * 
